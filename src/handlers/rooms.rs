...
use crate::{
    entities::room_members::RoomMember, errors::CustomError,
    repositories::room_member::RoomMemberRepository, request::Claims, AppState,
};
...
pub async fn get_room_members(
    ...
    if is_room_member {
        ...
        Ok((
            StatusCode::OK,
            response::Json(json!({ "room": room_members })),
        ))
    } else {
...
pub async fn delete_room_member(
    claims: Claims,
    Path((room_id, member_id)): Path<(i32, i32)>,
    State(state): State<Arc<AppState>>,
) -> Result<impl IntoResponse, CustomError> {
    // リクエストを送信したユーザーがroom_idのルームのメンバーかチェックする
    let room_members = state.room_member_repository.find_members(room_id).await;
